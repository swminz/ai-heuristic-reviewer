...
st.caption("Upload 1–5 screenshots. Get a professional heuristic review with annotated callouts, scorecard, and impact/effort chart.")
...
with st.sidebar:
    ...
    context = st.text_area("Context (optional)", placeholder="e.g., Mobile checkout for first-time users; success = task completion")
...
if submit_btn and uploads:
    ...
    for i, f in enumerate(uploads, start=1):
        ...
        w, h = img.size
        content_parts.append({"type":"text", "text": f"Screen {i} resolution: {w}x{h}. The screen shows UI elements for analysis."})

    with st.spinner("Analyzing with OpenAI…"):
        try:
            user_text = "Prepare heuristic review using the rubric. Screens described above. Return JSON and Markdown separated by delimiters."
    ...
    if raw:
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
            zf.writestr("report/report.md", report_md)
            screen_map = {}
            for i, f in enumerate(uploads, start=1):
                f.seek(0)
                img = Image.open(f).convert("RGB")
                screen_map[f"screen_{i}"] = (f.name, img.copy())
            ...
            for i, f in enumerate(uploads, start=1):
                f.seek(0)
                zf.writestr(f"images/original_{i}_{f.name}", f.read())
        scores = data.get("meta", {}).get("scores", {})
        H = scores.get("heuristics", {})
        ...
        def fmt_issue(iss):
            ...
            ac = "\\n".join([f"  - {c}" for c in rec.get("acceptance_criteria", [])])
...
"""
        ...
        final_report = REPORT_TEMPLATE_MD.format(
            ...
            h1=H.get("H1",""), h2=H.get("H2",""), h3=H.get("H3",""), h4=H.get("H4",""), h5=H.get("H5",""), h6=H.get("H6",""), h7=H.get("H7",""), h8=H.get("H8",""), h9=H.get("H9",""), h10=H.get("H10",""),
            annotated_images="\n\n".join(annotated_images_md) or "(annotated images included in ZIP)",
            findings=findings_md,
            impact_effort=impact_effort_md,
            num_screens=len(uploads)
        )
        st.subheader("Report Preview")
        st.markdown(final_report)
        with zipfile.ZipFile(zip_buffer, 'a', zipfile.ZIP_DEFLATED) as zf:
            zf.writestr("report/report_compiled.md", final_report)
        st.download_button("⬇️ Download report + images (ZIP)", data=zip_buffer.getvalue(), file_name="heuristic_review.zip")
